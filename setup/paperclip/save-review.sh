#!/bin/bash
# Haalt de laatste Review Agent comment op uit Paperclip en slaat hem op als bestand.
# Slaat over als het bestand al bestaat (idempotent).

API_URL="http://127.0.0.1:3100"
REVIEW_AGENT_ID="860d6694-7c33-4d9e-a12e-2fddd861f1cc"
COMPANY_ID="2799ab32-12fa-4d5e-8905-89fc9c81073d"
REPORTS_DIR="/home/niels/Projects/hyperliquid-bot/trading-company/reports/review"

# Haal de meest recente afgeronde review issue op
ISSUE_ID=$(curl -s "$API_URL/api/companies/$COMPANY_ID/issues?assigneeAgentId=$REVIEW_AGENT_ID&status=done" \
...
FILENAME="$REPORTS_DIR/${TIMESTAMP}-review.md"
...
echo "$(date '+%Y-%m-%d %H:%M') Opgeslagen: $FILENAME" >> /tmp/save-review.log
